...
if (isset($_GET['id'])) {
    ...
    if ($resultLeave && mysqli_num_rows($resultLeave) > 0) {
        ...
        $finishDate = $leaveData['finish_date'];
    } else {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['csrf_token']) && isCsrfTokenValid($_POST['csrf_token'])) {
      ...
      if (!empty($finishDate)) {
        ...
        if (empty($fullnameErr) && empty($divisionErr) && empty($reasonErr) && empty($categoryErr) && empty($finishDateErr)) {
            // Tentukan apakah ini adalah operasi penambahan data baru atau pembaruan
            if ($leave_id === null) {
                $insertQuery = "INSERT INTO leaves (user_id, division_id, reason, category, start_date, finish_date, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt = mysqli_prepare($conn, $insertQuery);
                if ($stmt) {
                    $created_by = $user_id;  // Gunakan user_id dari sesi
                    mysqli_stmt_bind_param($stmt, "iisssis", $fullname, $division, $reason, $category, $startDate, $finishDate, $created_by);
                }
            } else {
                ...
                if ($stmt) {
                    $updated_by = $user_id;  // Gunakan user_id dari sesi
                    mysqli_stmt_bind_param($stmt, "iisssiii", $fullname, $division, $reason, $category, $startDate, $finishDate, $updated_by, $leave_id);
                }
            }
